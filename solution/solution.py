...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return min(1.0, max(0.0, ap_sum / total_relevant))
    ...
    def run_full_eval(
        ...
        # Build a minimal QAPair for reference
        qa_pair = QAPair(
            question=question,
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Return ONLY a JSON object with criterion names as keys and float scores as values.\n"
        ...
        try:
            ...
            # Fall back to default score of 0.5 for each criterion
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Collect all scores across all batches
        ...
        # Positional bias: first response has notably higher average than the rest
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Pass pair.retrieved_contexts as the optional contexts argument
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_metric = min(scores, key=scores.get)  # type: ignore[arg-type]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggested_fix = suggestions[i] if i < len(suggestions) else "Review pipeline"
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        # Add suggestions based on dominant failure types
        if categories.get("hallucination", 0) > 0:
            suggestions.append(
                "Implement hallucination checker to filter unsupported claims and strengthen faithfulness guardrails"
            )
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Refine prompt template to more precisely direct the agent to address the user's specific question"
            )
        if categories.get("incomplete", 0) > 0:
            suggestions.append(
                "Increase chunk size in RAG pipeline to reduce context fragmentation and improve answer completeness"
            )
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Improve intent detection and add topic-boundary guardrails to keep answers on-topic"
            )
        if categories.get("refusal", 0) > 0:
            suggestions.append(
                "Calibrate safety guardrails to reduce over-refusal; add few-shot examples of legitimate responses"
            )

        # Always ensure at least 3 suggestions
        default_suggestions = [
            "Add few-shot examples showing complete, well-cited answers to improve overall quality",
            "Expand the golden dataset with more diverse and adversarial cases to improve benchmark coverage",
            "Implement continuous evaluation in CI/CD pipeline to catch regressions before deployment",
        ]
